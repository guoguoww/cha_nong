    ...
    <div>
      ...
      <div class="tuangou_pro_wrap">
        <ul >
          <li class="tuangou_content" v-for="(item,index) of tuangouProInfos">
            <p>
              <img :src="item.imgLoadPath" alt="">
            </p>
            <div>
              <p class="tuangou_tit_name"><span>{{item.goodsName}}</span></p>
              <p><span>认购量/总量：</span><span>{{item.subGoodsLeftNum}}/{{item.goodsTotalNum}}</span></p>
              <p><span>组一：</span><span>{{item.subGoodsSpec}}</span></p>
      ...
      <div v-show="curSta" class="tuangou_buy_wrap">
        <div class="tuangou_buy_info">
          ...
          <p><span>总金额 : </span><span class="commonRed">{{proTotal}}</span></p>
